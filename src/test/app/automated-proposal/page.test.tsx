...
/***
 * Esse teste está com erro pois o componente que engloba o select, não é um select puro.
 * O jest só consegue testar componentes puros do HTML.
 * Por esse motivo, alguns testes foram comentados até uma refatoração.
 */
...
// it("Teste para verificar Regime ao selecionar Estadual", async () => {
//   const regimeOption = {
//     GERAL: "GERAL",
//     ESPECIAL: "ESPECIAL",
//   };
...
//  const esferaSelect = await screen.findAllByText("Federal");
//  userEvent.click(esferaSelect[0]);

//  const optionEstadual = await screen.findByText("Estadual");
//  userEvent.click(optionEstadual);

//   const regimeLabel = await screen.findByLabelText("Regime");
...
// });
  ...
  /***
   * Esse teste deveria verificar todos os tribunais, mas o mesmo está com erro, pois o componente que engloba o select, não é um select puro.
   */
  ...
  //   for (const value of Object.values(tribunais)) {
  //     const option = await screen.findByRole("option", { name: value.nome });
  //     expect(option).toBeInTheDocument();
  //   }

  //   await Promise.all(
  //     tribunais.map(async (tribunal) => {
  //       const option = await screen.findByRole("option", { name: tribunal.nome });
  //       expect(option).toBeInTheDocument();
  //     })
  //   );
  ...
  it("Teste para verificar valor principal", async () => {
    const valorPrincipal = await screen.findByText("Valor Principal") as HTMLInputElement;
