...
describe("Testes na Redefinição de Senha", () => {
	...
	it("Teste para carregar as labels do formulário", async () => {
	...
	it("Teste para inserir e redefinir a senha", async () => {
		...
		const inputSenha = screen.getAllByLabelText(/Nova senha/i) as HTMLInputElement[];
		const inputConfirmarSenha = screen.getAllByLabelText(/Repita a nova senha/i) as HTMLInputElement[];
     ...
     expect(inputSenha[0].value).toBe("testeNovaSenha");
		...
		fireEvent.change(inputConfirmarSenha[0], { target: { value: "testeNovaSenha" } });
