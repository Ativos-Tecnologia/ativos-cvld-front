...
describe("Testes na Redefinição de Senha", () => {
  ...
  describe("Teste de erro do input do formulário", () => {
    it("Teste de erro ao redefinir a senha com três digitos", async () => {
      const inputSenha = screen.getAllByLabelText(
        /Nova senha/i,
      ) as HTMLInputElement[];
      const inputConfirmarSenha = screen.getAllByLabelText(
      ...
      fireEvent.change(inputConfirmarSenha[0], { target: { value: 123 } });
      expect(inputConfirmarSenha[0].value).toBe("123");

      await userEvent.click(botao);

      const erro = await screen.findByText("As senhas não conferem");
      expect(erro).toBeInTheDocument();
    });

    it("Teste de erro ao redefinir a senha com senhas diferentes", async () => {
      const inputSenha = screen.getAllByLabelText(
        /Nova senha/i,
      ) as HTMLInputElement[];
      const inputConfirmarSenha = screen.getAllByLabelText(
      ...
      fireEvent.change(inputSenha[0], { target: { value: 123456 } });
      expect(inputSenha[0].value).toBe("123456");
