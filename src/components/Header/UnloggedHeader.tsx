...
const UnloggedHeader = ({ logoPath, colorType }: IUnloggedHeaderProps) => {

    const [headerType, setHeaderType] = useState<'smooth' | 'glass' | 'solid'>('smooth');
    ...
    return (
        <>
            {/* header */}
            <div className={`fixed top-0 w-full z-1 ${headerType === 'smooth' ? 'bg-gray-200' : 'bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-10 bg-boxdark-2'} py-6 xsm:px-5 lg:px-10 transition-colors duration-500`}>
                <div className='max-w-230 mx-auto flex items-center justify-between'>
                    <Image
                        className="block"
                        src={logoPath}
                        alt="Logo"
