...
export function UserShadFilter() {
    ...
    return (
        <div className="flex w-full flex-col bg-white text-bodydark2 dark:bg-boxdark dark:text-bodydark">
            <div className="relative flex w-full">
                <label className="mb-2 flex bg-white text-sm font-semibold text-bodydark2 dark:bg-boxdark dark:text-bodydark">
                    <BiUser className="mr-2 h-5 w-5" />{' '}
                    <p className="uppercase">Filtro por usuário</p>
                </label>
                {loadingCardData && <AiOutlineLoading className="ml-4 animate-spin" />}
            </div>
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger className="bg-white dark:bg-boxdark-2" asChild>
                    <Button
                        variant="outline"
                        role="combobox"
                        aria-expanded={open}
                        className="justify-between 2xsm:w-full xl:w-[215px]"
                    >
                        {value ? value : 'Selecione o Usuário'}
                        <ChevronsUpDown className="opacity-50" />
                    </Button>
                </PopoverTrigger>
                <PopoverContent side="bottom" className="p-0 2xsm:w-full xl:w-[215px]">
                    <Command>
