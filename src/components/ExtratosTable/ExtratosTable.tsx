...
export function ExtratosTable({ newItem }: ExtratosTableProps) {
    ...
    const [extratosTableToNotionDrawersetId, setExtratosTableToNotionDrawersetId] =
        useState<string>('');
    ...
    const { filterData, resetFilters } = useFilter(
        data,
        setData,
        setStatusSelectValue,
        setOficioSelectValue,
        ...
        role === 'ativos' ? (
          ...
          <div className="p-5 bg-white rounded-sm dark:bg-boxdark">
            ...
            {window.innerWidth >= 100 ? (
                <>
                    {/* desktop view */}
                    <div className="rounded-sm bg-white p-5 dark:bg-boxdark">
                          ...
                          filterData={filterData} />
                                        ...
                                        <div className="flex items-center gap-1">
                                            <div
                                                ...
                                                className={`flex h-7 w-7 items-center justify-center rounded-full ${viewOption.type === 'table' && 'bg-slate-200 dark:bg-slate-600'} group cursor-pointer transition-colors duration-200 hover:bg-slate-200 dark:hover:bg-slate-600`}
                                            >
                                                <PiTable className="text-xl group-hover:text-black-2 dark:group-hover:text-white" />
                                            </div>

                                            {/* separator */}
                                            <div className="mx-1 h-5 w-px bg-zinc-300 dark:bg-form-strokedark"></div>
                                            {/* separator */}

                                            <div
                                                title="Mudar para visualização cards"
                                                onClick={() => setExtractListView('cards')}
                                                className={`flex h-7 w-7 items-center justify-center rounded-full ${viewOption.type === 'cards' && 'bg-slate-200 dark:bg-slate-600'} group cursor-pointer transition-colors duration-200 hover:bg-slate-200 dark:hover:bg-slate-600`}
                                            >
                        ...
                        {activedTab === 'WORKSPACE NOTION' && (
                            <NotionTableView
                                count={notionWorkspaceData?.results.length}
                                setExtratosTableToNotionDrawersetId={
                                    setExtratosTableToNotionDrawersetId
                                }
                ...
                /* mobile view */
                <div className="rounded-sm bg-white px-5 py-7 dark:bg-boxdark">
                    <div className="flex flex-col">
                        {/* tabs */}
                        <TableTabs resetFilters={resetFilters} />
                        {/* end tabs */}
