...
const CalcForm = ({
    ...
    React.useEffect(() => {
        if (data && formMode === "update") {
            ...
            setValue("valor_principal", numberFormat(data.properties["Valor Principal"].number || 0));
            ...
            setValue("valor_aquisicao_total", data.properties["Percentual a ser adquirido"].number! === 1);
            setValue("percentual_a_ser_adquirido", data.properties["Percentual a ser adquirido"].number! * 100);
            setValue("ja_possui_destacamento", data.properties["Honorários já destacados?"].checkbox);
            setValue("percentual_de_honorarios", data.properties["Percentual de Honorários Não destacados"].number! * 100 || 0);
            setValue("nao_incide_selic_no_periodo_db_ate_abril", data.properties["Incide Selic Somente Sobre Principal"].checkbox)
            ...
            setValue("npu_originario", data?.properties?.["NPU (Originário)"].rich_text?.[0]?.text.content || "");
            setValue("ente_devedor", data.properties["Ente Devedor"].select?.name || "");
            setValue("estado_ente_devedor", data.properties["Estado do Ente Devedor"].select?.name || "");
                        ...
                        <div className="flex flex-col gap-2 2xsm:col-span-2 2xsm:mt-3 sm:col-span-1 sm:mt-0">
                            <div className="relative flex flex-col justify-between">
                                <label
                                    ...
                                    className="mb-1 font-nexa text-xs font-semibold uppercase text-meta-5 truncate"
                                >
              ...
              /> */}
                ...
                /> */}
                    ...
                    {watch("ir_incidente_rra") &&
                        ...
                        <>
                            {(!watch("ir_incidente_rra") && watch("incidencia_rra_ir") && watch("natureza") === "NÃO TRIBUTÁRIA") ? (
                                <div className="col-span-1 hidden md:block"></div>
                            ) : null}
                        </>
                    )}
              ...
              /> */}
                    ...
                    <div className="col-span-2 flex flex-col gap-2">
                        ...
                        <div className="mt-8 flex flex-col gap-2">
                            {((watch("gerar_cvld") && formMode === "create") || formMode === "update") ? (
                                <>
                                        ...
                                        {watch("esfera") &&
                                            watch("esfera") !== "FEDERAL" && (
                                                <div className="flex w-full flex-col gap-2 2xsm:col-span-2 sm:col-span-1">
                                                    ...
                                                    <CelerAppCombobox
                                                        list={estados}
                                                        onChangeValue={(value) => setValue("estado_ente_devedor", value)}
                                                        value={estados.filter((estado) => estado.id === watch("estado_ente_devedor"))[0]?.nome || ""}
                                                    ...
                                                    {(watch("novo_usuario") === false ||
                                                        ...
                                                        watch("vincular_usuario") === true && (
                                                            ...
                                                            >
                                                                ...
                                                                {usersList
                                                                    .filter((user) => user !== userData.user)
                                                                    .map((user) => (
