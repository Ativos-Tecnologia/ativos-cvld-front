...
const CRMTooltip = ({
    ...
    return (
        <div className="group relative">
            {children}
            <div
                className={`absolute mb-3 ${tooltipPlacement[placement].container} invisible z-999999 whitespace-nowrap rounded-md border border-stroke bg-snow px-3.5 py-1 text-sm font-medium opacity-0 transition-all group-hover:visible group-hover:opacity-100 dark:border-strokedark dark:bg-black-2 duration-${animationDelay}`}
            >
                {arrow && (
                    <span
                        className={`absolute -z-10 h-2 w-2 ${tooltipPlacement[placement].arrow} rotate-45 border-stroke bg-snow dark:border-strokedark dark:bg-black-2`}
                    ></span>
                )}
                <p>
                    {text.split('\n').map((item, index) => (
