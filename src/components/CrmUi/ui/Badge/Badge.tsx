 ...
 */
const Badge: React.FC<BadgeProps> & { Label: React.FC<BadgeSubcomponentProps>; } = ({ color = '#97C9F0', children, isANotionPage = false }: BadgeProps): JSX.Element => {

  return (
    <div
      style={{ background: isANotionPage ? notionColorResolver(color) : color }}
      className='py-1 px-2 uppercase rounded-md text-black-2 text-[10px] font-semibold'
    >
      {children}
    </div>
  );
