...
const MarketplaceRecommendations: React.FC<MarketplaceRecommendationsProps> = ({ id }) => {
    ...
    function handleRentabilidadeTotal(valor_projetado: number, valor_investido: number) {
        const result = (valor_projetado - valor_investido) / valor_investido;
    ...
    return (
        <div>
            <h3 className="my-4 text-center uppercase tracking-wider text-black dark:text-snow">
                Outras oportunidades de investimento
            </h3>
            <div className="data-stats-slider-outer relative min-h-47.5 w-full max-w-full rounded-sm border border-stroke bg-white py-4 shadow-default dark:border-strokedark dark:bg-boxdark">
                <Swiper
                        ...
                        <>
                            {data?.map((item, index) => (
                                <SwiperSlide
                                    ...
                                    key={index}
                                    className="group:hover:bg-boxdark-2/35 cursor-pointer border-r border-stroke px-10 py-4 ease-in-out last:border-r-0 hover:bg-gray-100 dark:border-strokedark dark:hover:bg-boxdark-2/35"
                                >
                                    <div className="flex items-center justify-between ">
                                                    ...
                                                    <span
                                                        style={{
                                                            backgroundColor:
                                                                iconsConfig[
                                                                    data[index]
                                                                        ?.tipo as keyof typeof iconsConfig
                                                                ].bgColor,
                                                        }}
                                        ...
                                        <SmallProjectedProfitabilityChart
                                            updateValue={item.valor_liquido_disponivel}
                                            projectedValue={item.valor_projetado}
                                            updateDate={item.data_atualizacao}
                                            projectedDate={item.previsao_de_pagamento}
                                            monthsToPayment={Math.floor(
                                                handleMesesAteOPagamento(
                                                    item.data_atualizacao,
                                                    item.previsao_de_pagamento,
                                                ),
                                            )}
                                            investedValue={item.valor_investido}
                                            rentabilityPerYer={
                                                handleRentabilideAA(
                                                    handleRentabilidadeTotal(
                                                        item.valor_projetado,
